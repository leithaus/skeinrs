...
final class SpigotEngine: ObservableObject {
    ...
    func disconnect() {
        connectTask?.cancel()
        writeFH?.closeFile()
        if connectedFD >= 0 { Darwin.close(connectedFD); connectedFD = -1 }
        isConnected = false
    }
    ...
    private func openSocket() async throws {
        ...
        self.writeFH     = FileHandle(fileDescriptor: connFd, closeOnDealloc: false)
    }
